...
public class DuaArafahDataSource {


    SilaheMominSQLHelper msilaheMominSQLHelper;
    SQLiteDatabase msqLiteDatabase;

    public DuaArafahDataSource(Context context) {
        ...
        try {
            ...
            if (arrayListSqllite != null && arrayListSqllite.size() > 0) {
               return getListFromSQLite();
                ...
                arrayListLiveData = duaParser.getParseDua(C.DuaArafahJson);
                deleteAll();
    ...
    public ArrayList<Dua> getListFromSQLite() {
        ...
        try {
            Cursor mcursor = msqLiteDatabase.rawQuery(DuaArafahContract.SQL_SELECT_ALL, null);
            while (mcursor.moveToNext()) {
                Dua dua = new Dua();
                dua.setDua_arabic_part(mcursor.getString(mcursor.getColumnIndex(DuaArafahContract.DuaArafahEntry.COLUMN_NAME_ARABIC_PART)));
                dua.setDua_urdu_part(mcursor.getString(mcursor.getColumnIndex(DuaArafahContract.DuaArafahEntry.COLUMN_NAME_URDU_PART)));
                arrayList.add(dua);
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public long insert(Dua dua) {
        ...
        try {
            ...
            contentValues.put(DuaArafahContract.DuaArafahEntry.COLUMN_NAME_ARABIC_PART, dua.getDua_arabic_part());
            contentValues.put(DuaArafahContract.DuaArafahEntry.COLUMN_NAME_URDU_PART, dua.getDua_urdu_part());
            result = msqLiteDatabase.insert(DuaArafahContract.DuaArafahEntry.TABLE_NAME, null, contentValues);
    ...
    public long deleteAll() {
        try {
            msqLiteDatabase.execSQL(DuaArafahContract.SQL_DELETE_ALL);
