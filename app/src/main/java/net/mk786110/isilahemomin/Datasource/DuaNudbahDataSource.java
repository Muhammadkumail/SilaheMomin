...
public class DuaNudbahDataSource {


    SilaheMominSQLHelper msilaheMominSQLHelper;
    SQLiteDatabase msqLiteDatabase;

    public DuaNudbahDataSource(Context context) {
        ...
        try {
            ...
            if (arrayListSqllite != null && arrayListSqllite.size() > 0) {
               return getListFromSQLite();
                ...
                arrayListLiveData = duaParser.getParseDua(C.DuaNudbahJson);
                deleteAll();
    ...
    public ArrayList<Dua> getListFromSQLite() {
        ...
        try {
            Cursor mcursor = msqLiteDatabase.rawQuery(DuaNudbahContract.SQL_SELECT_ALL, null);
            while (mcursor.moveToNext()) {
                Dua dua = new Dua();
                dua.setDua_arabic_part(mcursor.getString(mcursor.getColumnIndex(DuaNudbahContract.DuaNudbahEntry.COLUMN_NAME_ARABIC_PART)));
                dua.setDua_urdu_part(mcursor.getString(mcursor.getColumnIndex(DuaNudbahContract.DuaNudbahEntry.COLUMN_NAME_URDU_PART)));
                arrayList.add(dua);
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public long insert(Dua dua) {
        ...
        try {
            ...
            contentValues.put(DuaNudbahContract.DuaNudbahEntry.COLUMN_NAME_ARABIC_PART, dua.getDua_arabic_part());
            contentValues.put(DuaNudbahContract.DuaNudbahEntry.COLUMN_NAME_URDU_PART, dua.getDua_urdu_part());
            result = msqLiteDatabase.insert(DuaNudbahContract.DuaNudbahEntry.TABLE_NAME, null, contentValues);
    ...
    public long deleteAll() {
        try {
            msqLiteDatabase.execSQL(DuaNudbahContract.SQL_DELETE_ALL);
