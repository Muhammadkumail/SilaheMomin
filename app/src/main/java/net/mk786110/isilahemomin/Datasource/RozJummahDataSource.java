...
public class RozJummahDataSource {
    SilaheMominSQLHelper msilaheMominSQLHelper;
    SQLiteDatabase msqLiteDatabase;

    public RozJummahDataSource(Context context) {
    ...
    public ArrayList<Dua> getList() {
        ...
        try {
            ...
            if (arrayListSqllite != null && arrayListSqllite.size() > 0) {
                ...
                arrayListLiveData = duaParser.getParseDua(C.RozJummahJson);
                deleteAll();
    ...
    public ArrayList<Dua> getListFromSQLite() {
        ...
        try {
            Cursor mcursor = msqLiteDatabase.rawQuery(RozJummahContract.SQL_SELECT_ALL, null);
            while (mcursor.moveToNext()) {
                Dua dua = new Dua();
                dua.setDua_arabic_part(mcursor.getString(mcursor.getColumnIndex(RozJummahContract.RozJummahEntry.COLUMN_NAME_ARABIC_PART)));
                dua.setDua_urdu_part(mcursor.getString(mcursor.getColumnIndex(RozJummahContract.RozJummahEntry.COLUMN_NAME_URDU_PART)));
                arrayList.add(dua);
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public long insert(Dua dua) {
        ...
        try {
            ...
            contentValues.put(RozJummahContract.RozJummahEntry.COLUMN_NAME_ARABIC_PART, dua.getDua_arabic_part());
            contentValues.put(RozJummahContract.RozJummahEntry.COLUMN_NAME_URDU_PART, dua.getDua_urdu_part());
            result = msqLiteDatabase.insert(RozJummahContract.RozJummahEntry.TABLE_NAME, null, contentValues);
    ...
    public long deleteAll() {
        try {
            msqLiteDatabase.execSQL(RozJummahContract.SQL_DELETE_ALL);
